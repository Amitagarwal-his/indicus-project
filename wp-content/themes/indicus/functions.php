...
function indicus_setup()
{
	...
	register_nav_menus(
		array(
			...
			'header' => esc_html__('Header', 'indicus'),
		)
	);
...
// Enqueue styles
	...
	wp_enqueue_script('gsap', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.8.0/gsap.min.js', array(), '3.8.0', true);
	wp_enqueue_script('scrolltrigger', 'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.8.0/ScrollTrigger.min.js', array('gsap'), '3.8.0', true);
}
...
function enqueue_homepage_script() {
    if ( is_front_page() ) {
       wp_enqueue_script('indicus-homepage-navigation', get_template_directory_uri() . '/js/menu-animation.js', array(), _S_VERSION, true);
    }
}
	...
	if ($allproduct) : ?>
		<div class="warr-pro-list"><?php
				...
				?>
					...
					<div class="warr-pro-number d-flex flex-wrap" data="<?php echo $url; ?>">
						<?php if ($image) : ?>
							<div class="war-img">
							...
							<?php if ($guide) {  ?>
								<a href="<?php echo esc_html($url); ?>" target="_blank" class="btn-fill">
									<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
										<path fill-rule="evenodd" clip-rule="evenodd" d="M0.625 12.3749C0.79076 12.3749 0.949732 12.4407 1.06694 12.5579C1.18415 12.6751 1.25 12.8341 1.25 12.9999V16.1249C1.25 16.4564 1.3817 16.7743 1.61612 17.0088C1.85054 17.2432 2.16848 17.3749 2.5 17.3749H17.5C17.8315 17.3749 18.1495 17.2432 18.3839 17.0088C18.6183 16.7743 18.75 16.4564 18.75 16.1249V12.9999C18.75 12.8341 18.8158 12.6751 18.9331 12.5579C19.0503 12.4407 19.2092 12.3749 19.375 12.3749C19.5408 12.3749 19.6997 12.4407 19.8169 12.5579C19.9342 12.6751 20 12.8341 20 12.9999V16.1249C20 16.7879 19.7366 17.4238 19.2678 17.8926C18.7989 18.3615 18.163 18.6249 17.5 18.6249H2.5C1.83696 18.6249 1.20107 18.3615 0.732233 17.8926C0.263392 17.4238 0 16.7879 0 16.1249V12.9999C0 12.8341 0.065848 12.6751 0.183058 12.5579C0.300269 12.4407 0.45924 12.3749 0.625 12.3749Z" fill="white"></path>
										<path fill-rule="evenodd" clip-rule="evenodd" d="M9.55825 14.8175C9.61631 14.8757 9.68528 14.9219 9.76121 14.9534C9.83714 14.9849 9.91855 15.0011 10.0008 15.0011C10.083 15.0011 10.1644 14.9849 10.2403 14.9534C10.3162 14.9219 10.3852 14.8757 10.4433 14.8175L14.1933 11.0675C14.3106 10.9501 14.3765 10.791 14.3765 10.625C14.3765 10.459 14.3106 10.2999 14.1933 10.1825C14.0759 10.0651 13.9167 9.99921 13.7508 9.99921C13.5848 9.99921 13.4256 10.0651 13.3083 10.1825L10.6258 12.8663V1.875C10.6258 1.70924 10.5599 1.55027 10.4427 1.43306C10.3255 1.31585 10.1665 1.25 10.0008 1.25C9.83499 1.25 9.67602 1.31585 9.55881 1.43306C9.4416 1.55027 9.37575 1.70924 9.37575 1.875V12.8663L6.69326 10.1825C6.5759 10.0651 6.41672 9.99921 6.25076 9.99921C6.08479 9.99921 5.92561 10.0651 5.80826 10.1825C5.6909 10.2999 5.62497 10.459 5.62497 10.625C5.62497 10.791 5.6909 10.9501 5.80826 11.0675L9.55825 14.8175Z" fill="white"></path>
									</svg> WARRANTY GUIDE
								</a>
							<?php } ?>
						</div>
...
// adding dynamic class for navigation
